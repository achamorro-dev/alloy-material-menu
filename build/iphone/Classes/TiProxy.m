...
//Rare exceptions to indicate a bug in the _alloy_material_menu code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_alloy_material_menu = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_alloy_material_menu]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.alloy_material_menu.module"
