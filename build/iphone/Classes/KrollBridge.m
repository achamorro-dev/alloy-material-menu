...
NSString * alloy_material_menu$ModuleRequireFormat = @"(function(exports){"
...
@implementation alloy_material_menuObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_alloy_material_menu);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create alloy_material_menu global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "alloy_material_menu" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_alloy_material_menu = [[alloy_material_menuObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_alloy_material_menu];
	
	NSString *_alloy_material_menuNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _alloy_material_menuNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_alloy_material_menu valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:alloy_material_menu$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.alloy_material_menu.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.alloy_material_menu.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
